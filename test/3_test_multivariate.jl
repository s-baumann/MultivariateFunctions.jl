...
mfun1 = PE_Function(1.8, make_unit_map([:x, :y, :z] .=> [PE_Unit(1.0,1.0,1), PE_Unit(1.0,1.0,1), PE_Unit(1.0,2.0,3)]))
abs(mfun1.multiplier_ - 1.2*1.5) < 1e-10
mfun2 = PE_Function(4.5, make_unit_map([:x, :y] .=> [PE_Unit(1.0,2.0,3), PE_Unit(1.0,3.0,2)]))
mfun3 = PE_Function(3.0, make_unit_map([:z, :y] .=> [ PE_Unit(1.0,3.0,2), PE_Unit(1.0,1.0,1)]))
mfun4 = PE_Function(6.0, make_unit_map([:x, :y] .=> [PE_Unit(0.0,1.0,1), PE_Unit(0.0,2.0,4)]))
mfun5 = PE_Function(6.0, make_unit_map([:x, :z] .=> [PE_Unit(0.0,1.0,1), PE_Unit(0.0,2.0,2)]))
mfun6 = PE_Function(18.0, make_unit_map([:x, :y, :z] .=> [PE_Unit(0.0,Date(2015,1,1),4), PE_Unit(0.0,1.0,1), PE_Unit(0.0,Date(2016,1,1),2)]))
...
function test_result_mv(func, eval_to)
...
test_result_mv(mfun1, mfun1_value)
mfun2_value = evaluate(fun3, coordinates[:x]) * evaluate(fun4, coordinates[:y])
test_result_mv(mfun2, mfun2_value)
test_result_mv(mSumFunction1, mfun1_value + mfun2_value)
mfun3_value = evaluate(fun4, coordinates[:z]) * evaluate(fun1, coordinates[:y])
test_result_mv(mSumFunction3, 2* mfun1_value + 2*mfun2_value + mfun3_value)
...
test_result_mv( ((7 - (5.0 - mfun1))  + 5.0) + 7        , 14 + mfun1_value                )
test_result_mv( ((7 - (5.0 - mSumFunction1))  + 5.0) + 7, 14 +  mfun1_value + mfun2_value )
# multiplications
test_result_mv( (5 * mfun1) * 7.0        , 35* mfun1_value                  )
test_result_mv( (5 * mSumFunction1) * 7.0, 35*  (mfun1_value + mfun2_value) )
# Divisions
test_result_mv( (mfun1/2) / 3.0       ,  mfun1_value/6                  )
test_result_mv( (mSumFunction1/2) /3.0, (mfun1_value + mfun2_value)/6 )
# powers
test_result_mv( mfun1^0, 1.0)
test_result_mv( mSumFunction1^0, 1)
test_result_mv( mfun1^1, mfun1_value)
test_result_mv( mSumFunction1^1, (mfun1_value + mfun2_value))
test_result_mv( mfun1^2, mfun1_value^2)
test_result_mv( mSumFunction1^2, (mfun1_value + mfun2_value)^2)
# multiplying functions
test_result_mv( mfun1 * mfun2, mfun1_value * mfun2_value)
# Using Dates
...
# Derivatives
test_result_mv(derivative(mfun4, Dict{Symbol,Int}(:x => 2)) , 0.0)
test_result_mv(derivative(mfun4, Dict{Symbol,Int}(:y => 4)), 24 * 6 * (-0.5) )
test_result_mv(derivative(mfun5, Dict([:x, :z] .=> [1,1])), 12 * (3-2)^1 )
# Integration
