function next_chebyshev(previous::Sum_Of_Functions, two_previous::Sum_Of_Functions, dim_name::Symbol)
    return PE_Function(2.0, UnitMap([dim_name => PE_Unit(0.0,0.0,1)])) * previous - two_previous
end
...
function get_first_kind_Chebyshevs(num::Int, dim_name::Symbol)
    if num < 1
        ...
        first_kind_chebyshevs[1] = Sum_Of_Functions([PE_Function(1.0, UnitMap([dim_name => PE_Unit(0.0,0.0,0)]))])
        if num > 1
            first_kind_chebyshevs[2] = Sum_Of_Functions([PE_Function(1.0, UnitMap([dim_name => PE_Unit(0.0,0.0,1)]))])
        end
        for n in 3:num
...
function get_second_kind_Chebyshevs(num::Int, dim_name::Symbol)
    if num < 1
        ...
        second_kind_chebyshevs[1] = Sum_Of_Functions([PE_Function(1.0, UnitMap([dim_name => PE_Unit(0.0,0.0,0)]))])
        if num > 1
            second_kind_chebyshevs[2] = Sum_Of_Functions([PE_Function(2.0, UnitMap([dim_name => PE_Unit(0.0,0.0,1)]))])
        end
        for n in 3:num
